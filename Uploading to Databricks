# SharePoint to Databricks Pipeline

End-to-end event-driven pipeline that uploads SharePoint files to a Databricks Unity Catalog Volume and triggers a job to ingest them into a Delta table.
...
- Personal Access Token with both `files` and `jobs` scopes
- Permissions on the target Volume (WRITE VOLUME) and Catalog/Schema (CREATE TABLE)
...
Note: avoid the deprecated `When a file is created in a folder` trigger.
...
Setting Infer Content Type to **No** is critical for binary files like Excel. When `Yes`, Power Automate tries to interpret the file and corrupts the binary content, resulting in a 0-byte file landing in the Volume.
...
```json
{
  ...
  "$content": "UEsDBBQABg..."  // base64-encoded file bytes
}
```
...
Path: `/Workspace/Users/dheeraj.tommandru@mssm.edu/bsc_finance_ingest`

```python
# Parameters — populated from Power Automate via job_parameters
...
|---|---|---|
| GET | Read data | List jobs, download a file |
| POST | Create / trigger | Trigger a job run |
| PUT | Replace / upload | Upload a file to a path |
| PATCH | Partial update | Modify a job's schedule |
...
Using `Get file content` (by ID) with the new `When a file is created (properties only)` trigger fails because the trigger outputs a list item ID instead of a file ID. Use `Get file content using path` and pass the `Full path` dynamic value instead.

## Token Management

| Scope | What it allows |
|---|---|
| `files` | Upload, download, delete files via `/api/2.0/fs/files/...` |
| `jobs` | Trigger, cancel, inspect jobs via `/api/2.1/jobs/...` |
| `sql` | Run SQL warehouse queries |
| `unity-catalog` | Manage Unity Catalog objects |

Best practice: use scoped tokens with only the permissions needed. For production, switch to a service principal with OAuth M2M tokens rather than a personal PAT.

## Future Improvements

- Replace PAT with a service principal OAuth token
- Add error handling and Slack alerting in Power Automate (Try / Catch parallel branch)
- Filter trigger by file extension (only `.xlsx`, `.csv`)
- Capture file metadata (uploader, upload time) as columns in the Delta table
- Add bronze → silver → gold layering instead of overwriting the bronze table
- Move from `overwrite` mode to incremental merge based on a business key
