...
Jobs should be documented with the expected [compute](../Common%20Definitions.md), source query, target output, and owner before they are scheduled.
...
6. Run the job once manually before turning on the schedule.
...
Use this pattern when the SQL output is an input step for a notebook that performs additional transformation, validation, or file generation.
...
3. Build the notebook step to read from that table or view and complete the downstream work.
4. Create a multi-task [Job](../Common%20Definitions.md) or a job with dependent tasks.
...
- Give the job a clear owner and a failure response path.
